...
@LastEditTime : 2021-01-31 04:11:31
'''
import uuid
import random
...
def login():
    ...
    post_password = get_md5(password)
    ...
    if results:
        ...
        if curr_password == post_password:
            ...
            return make_response(jsonify(r_data), 401.1)
    else:
        r_data = {"code": 1, "msg": "用户名或密码错误"}
        logger_msg(username=username, action="/api/user/login", data="登录失败 {}".format(post_data), msg_type="user")
        return make_response(jsonify(r_data), 401.1)
    ...
    try:
        with session_maker(rss_sqlite_uri) as db_session:
            ...
            for delete_id in delete_id_list:
                ...
                logger.info("delete user {}".format(delete_user_id))
                ...
                logger.info("delete user {} article: {}".format(delete_user_id, article_id_list))
                ...
                logger.info("delete user {} video: {}".format(delete_user_id, video_id_list))
                ...
                logger.info("delete user {} data".format(delete_user_id))
    except Exception as e:
        logger.error(str(e))
