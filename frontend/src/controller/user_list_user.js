    ...
    layui.use(['table', 'form', 'admin'], function(){
        ...
        var tableIns = table.render({
            ...
            cols: [[
                ...
                {field: 'username', title: 'UserName', sort: true, fixed: 'left'},
                {field: 'face', title: 'Face'},
                {field: 'role', title: 'Role'},
                {field: 'email', title: 'Email'},
                {field: 'add_time', title: 'Add Time'},
                {field: 'operate', title: 'Operate', width:162, fixed: 'right', align:'center', toolbar: '#table-bar'},
            ]],
            done : function () {
        ...
        table.on('toolbar(user-table)', function(obj){
            switch(obj.event){
                ...
                case 'delete-select':
                    ...
                    layer.confirm('确定删除 '+delete_id_list.length+'条数据?', {icon: 3, shadeClose: true}, function(index){
                        admin.req({
                            url: '/api/user/delete',
                            type: 'post',
                            dataType: "json", //期望后端返回json
                            contentType: "application/json", //发送的数据的类型
                            data: JSON.stringify({"delete_id_list": delete_id_list}),
                            timeout: 20000,
                            success: function (result) {
                                if (result.code == 0){
                                    tableIns.reload();
