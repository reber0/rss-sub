...
layui.define(['laytpl', 'layer'], function(exports){
    ...
    view.req = function(options){
      ...
      return $.ajax($.extend({
        ...
        //   console.log({
        //     "readyState": e.readyState,// 4
        //     "responseJSON": e.responseJSON, // {code: 401, msg: "登录失效"}
        //     "status": e.status, // 401
        //     "statusText": e.statusText,// "UNAUTHORIZED"
        //   });

          if (e.status == 400) {
            layer.msg(e.responseJSON.msg, {offset: '150px', icon: 2, time: 1000});
          } else if (e.status == 401){ // 登录失败和登录失效都返回 401，但是 msg 不同
            layer.msg(e.responseJSON.msg, {offset: '150px', icon: 2, time: 1000});
            view.exit();
            ...
            var errorText = [
              '请求异常，请重试<br><cite>状态码：</cite>'+ e.status + '<br><cite>错误信息：</cite>' + e.responseJSON.msg 
              ,debug()
              ].join('');
            view.error(errorText);
          } else {
            var errorText = [
              '请求异常，请重试<br><cite>状态码：</cite>'+ e.status + '<br><cite>错误信息：</cite>' + code
              ,debug()
              ].join('');
            view.error(errorText);
          }
